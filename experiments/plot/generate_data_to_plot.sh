...
if [ $# -ne 5 ]
then
	echo "Usage:" $0 "output_dir traces_file machines_file system out_file"
	exit 1
fi
...
# {30sec, 1min, 1day}/result_$sample/machine.$random.replay.out/
...
do
    ...
    do
        sample=`dirname $file | xargs basename | cut -d"_" -f2`
        stamp=`dirname $file | xargs dirname | xargs basename` #{30sec, 1min, 1day}
